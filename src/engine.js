...
let autoStart;
let playingText;
let autoStartSeconds;
let autoCloseReader;

// Word arrays and control constiables
let wordIndex;
let showSlideTime = 0;
...
// The selectedAlgorithm is set via constiable
...
let pauseAfterComma;
let pauseAfterPeriod;
let pauseAfterParagraph;
let wordFlicker;
let pauseAfterCommaDelay;
let pauseAfterPeriodDelay;
let pauseAfterParagraphDelay;
let wordFlickerPercent;

// Advanced display settings
let highlightOptimalLetter;
let highlightOptimalLetterColour;
...
let splitTextFirstPass;
let splitTextSecondPass;
let indexGlobal;
let exText;
...
// WPM constiables to adjust speed on the fly
let WPMAdjusted = 0;
let WPMAdjustedPadding = 0;
let WPMTimingAdjustmentMS = 0;
...
function splitTextToArray_FirstPass(selectedText) {
    ...
    const spacedText = htmlEntitiesEncode(selectedText);

    // --------------------------------------------------------------------
    // STEP 2
    // Split the selected text by single space character
    //console.log(spacedText);
    const splitText = spacedText.match(/\S+/g);

    // --------------------------------------------------------------------
    // STEP 3
    // Remove the last array element if it's blank
    let totalWords = splitText.length;
...
function splitTextToArray_FirstPass_CleanupLoop(index, splitText) {
    ...
    for (let i = index, count = splitText.length; i < count; i++) {
        ...
        const textOnSlide = textItemFirst.text;
        ...
        if (chunkSize === 1) {
            ...
            if (textOnSlide.indexOf("-") !== -1) {
                if (textOnSlide.length === 1) {
                    ...
                    const lastCharacter = textOnSlide.indexOf("\n");
                    if (lastCharacter !== -1 && textOnSlide[textOnSlide.length - 1] === "-") {
                        if (madvConsolidateHyphenatedWord === 'true') {
                            ...
                            exText = textOnSlide;
                            const result = joinHyphenatedWord(i, splitText);
                            if (result) {
            ...
            if (madvEnableLongWordHyphenation === 'true') {
                const textDecoded = htmlEntitiesDecode(textOnSlide);
                if (textDecoded.length > madvLongWordTriggerCharacterCount) {
                    // Split the word using hyphenate and language pattern
                    const splitFirst = $().hyphenateWord(textOnSlide, language.pattern, madvLongWordTriggerCharacterCount);
                    const splitWord = adjustStringArray(splitFirst, madvLongWordMinCharacterPerSlidePostSplit, madvLongWordTriggerCharacterCount);
                    ...
                    for (let x = 0; x < splitWord.length; x++) {
                        ...
                        if (x === splitWord.length - 1) {
                            ...
                            const lastTextSegment = splitWord[x];
                            if (lastTextSegment.length <= madvLongWordCharacterTriggerDoNotJoin) {
                                const lastWord = splitTextFirstPass[splitTextFirstPass.length - 1].text;
...
function joinHyphenatedWord(index, splitText) {
    ...
    if (index + 1 < splitText.length) {
        ...
        const nextWord = splitLength[index + 1];
...
function splitHyphenatedWord(index, splitText, textItemFirst) {
    ...
    const chrRegEx = /[a-z]/i;

    const splitHyphenated = text.split("-").reduce(function (result, current) {
    ...
    if (splitHyphenated.length > 0) {
        for (let i = 0, count = splitHyphenated.length; i < count; i++) {
            const tempText = {};

            tempText.childofprevious = i > 0;
            tempText.slidenumber = textItemFirst.slidenumber;
            tempText.textoriginal = textItemFirst.textoriginal;
            ...
            if (i === 0 && splitHyphenated.length === 2) {
                const combinedLength = splitHyphenated[0].length + splitHyphenated[1].length;
    ...
    if (index < splitText.length) {
       const text = splitText[index];
        ...
        if (text.slice(-1) === '.' && text.length >= 2 && isNaN(text[0])) {
            ...
            // The slide is a single character that is nor "I" or "A"
            // This will more than likely be the final character of an acronym
            const containsAbbStartingCharacter = text[0].match('/A|I/i');
            if (!containsAbbStartingCharacter) {
                exText = exText + text;
                indexGlobal = index;
            }
        }
...
function splitTextToArray_SecondPass_CleanupLoop(index, splitText) {
    ...
    for (let i = index, count = splitText.length; i < count; i++) {
        ...
        // Assign text to constables
        const text = splitText[i].text;
        const textOriginal = splitText[i].textoriginal;

        // Check for

        // The text from the slide
        const textItemSecond = {};
        ...
        // The text from the slide
        const textOnSlide = splitText[i].text;
...
function addCommaPauseAfterCharacter(character, text, textItem, splitTextArray) {
    const indexCharacter = text.lastIndexOf(character);
    if (indexCharacter !== -1) {
        ...
        // We have the character at the start
            ...
            if (pauseAfterComma === 'true') {
                splitTextArray[splitTextArray.length - 1].postdelay = pauseAfterCommaDelay;
            }
        }
    }
    return false;
    ...
    if (indexCharacter) {
        ...
        // We have the character at the start
            ...
            if (pauseAfterPeriod === 'true') {
                splitTextArray[splitTextArray.length - 1].postdelay = pauseAfterPeriodDelay;
            }
        }
    }
    return false;
...
function addParagraphPauseAfterCharacter(character, text, textItem, splitTextArray) {
    const indexCharacter = text.indexOf(character);
    if (indexCharacter !== -1) {
        // We have the character at the end
        if (indexCharacter === text.length - 1) {
        ...
        // We have the character at the start
            ...
            if (pauseAfterParagraph === 'true') {
                splitTextArray[splitTextArray.length - 1].postdelay = pauseAfterParagraphDelay;
            }
        }
        text.trim(character);
    }
...
function getTextArrayBasic(selectedText, chunkSize) {
    ...
    // Assign test text to the constable below
    ...
    const splitText = splitTextSecondPass;
    ...
    for (let i = 0; i < splitText.length; i++) {
        ...
        for (let j = 0; j < chunkSize; j++) {
            if (i + j < splitText.length) {
                ...
                // --------------------------
                // COMMA
                const comma = addCommaPauseAfterCharacter(',', text, textItem, splitTextArray);
                if (comma) break;

                // SEMI-COLON
                const semi = addCommaPauseAfterCharacter(';', text, textItem, splitTextArray);
                if (semi) break;

                // COLON
                const colon = addCommaPauseAfterCharacter(':', text, textItem, splitTextArray);
                if (colon) break;

                // --------------------------
                // PERIOD (Period .)
                const period = addPeriodPauseAfterCharacter('.', text, textItem, splitTextArray);
                if (period) break;

                // QUESTION MARK (Period ?)
                const question = addPeriodPauseAfterCharacter('?', text, textItem, splitTextArray);
                if (question) break;

                // APOSTROPHE (Period !)
                const apostrophe = addPeriodPauseAfterCharacter('!', text, textItem, splitTextArray);
                if (apostrophe) break;

                // --------------------------
                // PARAGRAPH
                const paragraph = addParagraphPauseAfterCharacter("\r\n", text, textItem, splitTextArray);
                if (paragraph) break;

                // NEW LINE
                const newLine = addParagraphPauseAfterCharacter("\n", text, textItem, splitTextArray);
                if (newLine) break;
            }
        }
    ...
    if (splitTextArray.length > 0) {
        const slide = splitTextArray[splitTextArray.length - 1];
        slide.postdelay = 0;
    }
...
function getTextArrayBasicTiming(textData) {
    ...
    // Basic algorithm all text items have
    // the same timing based on the following
    const duration = 1 / (WPM / 60) * 1000;

    for (let i = 0; i < textData.length; i++) {
        const t = textData[i];
        const wordsinslide = t.wordsinslide;
...
function getTextArrayWordLengthTiming(textData) {
    ...
    const totalSegments = textData.length;

    // Calculate the approx duration of the text display in ms
    const totalDuration = (totalSegments / WPM) * 60000;
    ...
    const unitTime = totalDuration / totalLength;
...
function getTextArrayWordFreqTiming(textData) {
    ...
    for (let i = 0; i < textData.length; i++) {
        const t = textData[i];
        let bitsOfInformation = -Math.log2(wordProbability[t.text]); // the Shannon information content definition

        // linear interpolation: 4.5 bits -> durShort msec, 25.6 bits -> durLong msec
        const lowInfo = 4.5;
        const highInfo = 25.6; // this numbers are empirical and are taken from the en-US database.
        const durShort = madvWordFreqHighestFreqSlideDuration;
        const a = (madvWordFreqLowestFreqSlideDuration - durShort) / (highInfo - lowInfo);
...
function playSlideShow() {
    ...
    if (textItemIndex === 1) {
        ...
        if (wordIndex >= textArray.length) {
            ...
            // Set the progress
            const p = Math.round((wordIndex / textArray.length) * 100);
            setProgress(p);
            // Display the word
    ...
    // Display blank word for postdelay (if timing requires)
        ...
        // the window can be auto closed if we are at the end
...
function displayBlankWord() {
    ...
    wordContainerElement.css('margin-right', "0");
...
function setWordLeftPadding(textItem) {
    ...
    if (textPosition === 2 || textPosition === 3) {
        ...
        let px = textItem.pixeloffsettooptimalletter;
...
function doWeReverseSlide(textItem) {
    ...
    const hasNumbers = /\d/.test(textItem.textoriginal);
    ...
    textItem.text = textItem.textoriginal.replace(/\$?[0-9]+(\.[0-9]+,[0-9]+)?%?/g, function (s) {
...
function highlightTheOptimalLetter(textItem) {
    ...
    if (highlightOptimalLetter === 'true' && chunkSize === 1) {
        ...
        let letterPosition = textItem.optimalletterposition;
        if (textItem.hasbeenreversed) {
            const newPosition = textItem.optimalletterposition - 1;
            if (newPosition >= 1) letterPosition = newPosition;
        }
        // Colour the Nth letter of the word
        const charClass = ".char" + letterPosition;
...
function assignOptimalLetterPosition(textItem) {
    // This algorithm will determine the letter based on word length
    const length = textItem.text.length;
    let position = 4;
...
function calculatePixelOffsetToOptimalCenter(textItem, isRightToLeft) {
    ...
    const index = textItem.optimalletterposition - 1;
    let wordToOptimalStart = "";
    let wordOptimalLetter = "";
    const length = textItem.text.length;
    ...
    const pxToOptimalStart = context.measureText(wordToOptimalStart).width;
    const pxOptimalLetter = context.measureText(wordOptimalLetter).width;
    ...
    return pxToOptimalStart + (pxOptimalLetter / 2);
}
...
function addStaticFocalToTextItem(textItem, isRightToLeft) {
    ...
    if (isRightToLeft) {
        sub1 = text.substring(0, text.length - textItem.optimalletterposition);
        sub2 = text.substring(text.length - textItem.optimalletterposition, text.length);
        textItem.optimalletterposition = textItem.optimalletterposition - 1;
    } else {
        sub1 = text.substring(0, textItem.optimalletterposition);
        sub2 = text.substring(textItem.optimalletterposition, text.length);
        textItem.optimalletterposition = textItem.optimalletterposition + 1;
    }
...
function setSlideShowData(textData) {
    ...
    for (let i = 0; i < textData.length; i++) {
        const d = textData[i].duration;
        const pd1 = textData[i].predelay;
        const pd2 = textData[i].postdelay;
    ...
    const totalAdjustedTiming = WPMTimingAdjustmentMS * textData.length;
...
function getMinAndSecondsString(milliseconds) {
    const ms = milliseconds.toFixed();
    const minFull = (ms / 1000) / 60;
    const minutes = Math.floor(minFull);
    const seconds = ((minFull - minutes) * 60).toFixed();
...
function adjustWPM(adjustmentAmount) {
    let newPadding = WPMAdjustedPadding + adjustmentAmount;
    const newWPM = Math.max(30, WPM + newPadding);
    if (newWPM === 30) newPadding = WPMAdjustedPadding;

    const origTiming = 60000 / WPM;
    const newTiming = 60000 / newWPM;
    //console.log('WPM Padding: ' + newPadding + ' | WPM: ' + newWPM + ' | Timing Adjustment: ' + newTimingAdjustment);

    WPMTimingAdjustmentMS = newTiming - origTiming;
